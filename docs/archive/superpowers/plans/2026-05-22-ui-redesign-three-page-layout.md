...
**Reference sources:**
- Legacy design system: `<USER_HOME>\Code\agentic-sovereign-ecosystem\apps\sovereign-vault\src\`
...
```bash
cd <USER_HOME>/Code/sovereign-vault/ui
...
```bash
mkdir -p <USER_HOME>/Code/sovereign-vault/ui/src/lib
mkdir -p <USER_HOME>/Code/sovereign-vault/ui/src/stores
mkdir -p <USER_HOME>/Code/sovereign-vault/ui/src/components
mkdir -p <USER_HOME>/Code/sovereign-vault/ui/src/pages
```

- [ ] **Step 3: Rename App.svelte to App.legacy.svelte**

```bash
cp <USER_HOME>/Code/sovereign-vault/ui/src/App.svelte \
   <USER_HOME>/Code/sovereign-vault/ui/src/App.legacy.svelte
```
...
```bash
cd <USER_HOME>/Code/sovereign-vault/ui && npm run check
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add -A
git -C <USER_HOME>/Code/sovereign-vault commit -m "chore(ui): scaffold three-page layout — add deps, dirs, App.legacy"
```
...
```bash
cp "<USER_HOME>/Code/agentic-sovereign-ecosystem/apps/sovereign-vault/src/app.css" \
   "<USER_HOME>/Code/sovereign-vault/ui/src/app.css"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/app.css ui/src/main.ts
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): port design system CSS from legacy sovereign-vault"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/lib/
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): add lib/types, lib/tauri invoke wrapper, lib/formatters"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/stores/
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): add six Svelte 5 rune stores (vault, containers, files, mcp, approvals, toast)"
```
...
`<USER_HOME>\Code\agentic-sovereign-ecosystem\apps\sovereign-vault\src\lib\ContextMenu.svelte`
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/components/
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): add atomic components (ModePill, CopyButton, Toast, ContextMenu)"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/components/SidebarNav.svelte ui/src/components/TopBar.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): port SidebarNav (3-item) and TopBar from legacy design"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/App.svelte ui/src/pages/ ui/src/components/LockedCard.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): wire App.svelte router shell with locked-state guard"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/components/LockedCard.svelte ui/src/components/RecoveryPhraseBanner.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): add LockedCard (init/unlock tabs) and RecoveryPhraseBanner"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/components/ContainerRow.svelte ui/src/components/NewContainerModal.svelte ui/src/pages/VaultPage.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): implement VaultPage — ContainerRow, NewContainerModal, vault list"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/components/FileRow.svelte ui/src/components/UploadDropzone.svelte ui/src/components/ApprovalBanner.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): add FileRow, UploadDropzone, ApprovalBanner components"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/components/FileViewerModal.svelte ui/src/components/ApprovalModal.svelte ui/src/components/OtpModal.svelte ui/src/pages/FilesPage.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): implement FilesPage — file table, modals, upload, approval flow"
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add ui/src/pages/SettingsPage.svelte
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): implement SettingsPage — identity, about, MCP server, approvals"
```
...
```bash
rm <USER_HOME>/Code/sovereign-vault/ui/src/App.legacy.svelte
```
...
```bash
cd <USER_HOME>/Code/sovereign-vault && cargo tauri build 2>&1 | tail -20
```
...
```bash
git -C <USER_HOME>/Code/sovereign-vault add -A
git -C <USER_HOME>/Code/sovereign-vault commit -m "feat(ui): complete three-page UI redesign — delete App.legacy, final clean"
```
