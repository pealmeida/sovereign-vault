...
|---|---|---|
| Release-mode benchmark runs + host/methodology table | §3.9.1 rigor | run `thesis-eval --release`, capture host specs; commit a results appendix |
| Component micro-measurements (isolated decrypt vs. filter) | §3.9.1 Eq. 1 completeness | add a sub-mode to the harness that times `sv-privacy::redact` and raw `read_file` directly |
