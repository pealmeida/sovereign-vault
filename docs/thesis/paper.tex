...
Três resultados organizam a leitura da Tabela~\ref{tab:latencia-pareada-v2}. O primeiro é positivo e central para a QP2: o custo do \textit{gate} de consentimento é indistinguível de zero. Nenhum dos seis contrastes pareados exclui zero. Sob AutoAllow, o experimento não distingue de zero o custo de APPROVAL e OTP sobre DIRECT; a ausência de rejeição não demonstra equivalência, e o desenho não mede a espera humana. Em produção, $T_{espera\ humana}$ permanece dominado pelo tempo de reação humana, tipicamente em segundos, parâmetro externo não medido pelo \textit{gateway}.
...
execução de código e análise; e ferramentas baseadas em GPT-6/Codex (OpenAI), empregadas
...
identificável são sintéticos, gerados para os ensaios e versionados em
\texttt{docs/thesis/evidence/}, incluindo o conjunto adicional descrito nas limitações.
